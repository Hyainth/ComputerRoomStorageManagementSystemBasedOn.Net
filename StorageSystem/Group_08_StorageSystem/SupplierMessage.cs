...
{
    public partial class SupplierMessage : Form
    {
        ...
        private void SupplierMessage_Load(object sender, EventArgs e)
        {
            ...
            if (useMan.Select("Supplier", out dt).Equals("NotFind"))
            { MessageBox.Show("未找到该表，请确认表名是否错误!", "提示"); Close(); }
            else if (useMan.Select("Supplier", out dt).Equals("Success"))
        ...
        {
            string flag = useMan.Delete("Supplier", dataGridView1.Columns[0].HeaderText, dataGridView1.CurrentRow.Cells[0].Value.ToString());
        ...
        {
            SupplierEdit supEdit = new SupplierEdit();
            supEdit.supplier.SupID = dataGridView1.CurrentRow.Cells[0].Value.ToString();
            supEdit.ShowDialog();
            tableFlash();        
        ...
        {
            DataTable dt = new DataTable();
            string flag = useMan.Select("Supplier", comboBoxSelect.Text, textSelect.Text, out dt);
            if (flag.Equals("Success")) { dataGridView1.DataSource = dt.DefaultView; }
            else
            {
